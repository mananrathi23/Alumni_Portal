...
// ── Accepted — to student ───────────────────────────────────────────────────
export function mentorshipAcceptedStudentEmail({ studentName, mentorName, mentorRole, goal, slotDay, slotTime }) {
  const goalLabel = { career:"Career Guidance", resume:"Resume Review", interview:"Interview Prep", technical:"Technical Help", general:"General Advice" }[goal] || goal;
...
<div class="wrap"><div class="card">
  ...
  <div class="body">
    ...
    <p>Your chat with ${mentorName} is now enabled. You can coordinate the meeting link and details directly through the chat on the Alumni Portal.</p>
...
// ── Accepted — to mentor ────────────────────────────────────────────────────
export function mentorshipAcceptedMentorEmail({ mentorName, studentName, studentDept, studentYear, goal, slotDay, slotTime }) {
  const goalLabel = { career:"Career Guidance", resume:"Resume Review", interview:"Interview Prep", technical:"Technical Help", general:"General Advice" }[goal] || goal;
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><style>${BASE_STYLES}</style></head><body>
<div class="wrap"><div class="card">
  ...
  <div class="body">
    ...
    <p>The chat with ${studentName} is now open. You can share a Google Meet or Zoom link, discuss agenda, and prepare for the session there.</p>
  </div>
...
// ── Auto-rejected (slot booked by another student) — to student ─────────────
...
// ── Session reminder (15 min before) — to both ─────────────────────────────
export function sessionReminderEmail({ recipientName, otherPersonName, otherPersonRole, slotDay, slotTime, meetingLink }) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><style>${BASE_STYLES}</style></head><body>
<div class="wrap"><div class="card">
  ...
  <div class="body">
    ...
    <div class="info-box" style="border-color:#667eea">
      ...
      ${meetingLink ? `<div class="info-row"><span class="info-label">Meeting Link</span><span class="info-value"><a href="${meetingLink}" style="color:#4c51bf">${meetingLink}</a></span></div>` : ""}
    </div>
    ${meetingLink
      ? `<a href="${meetingLink}" class="btn" style="background:#4c51bf;color:#fff;">Join Meeting Now →</a>`
      : `<p>Check your chat on the Alumni Portal for the meeting link shared by your ${otherPersonRole === "Student" ? "mentor" : "student"}.</p>`
    }
  </div>
