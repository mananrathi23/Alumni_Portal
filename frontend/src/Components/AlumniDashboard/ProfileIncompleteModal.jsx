...
const ProfileIncompleteModal = ({ alumni, onClose }) => {
  const navigate = useNavigate();
  const goToProfile = () => { onClose(); navigate("/alumni/profile"); };
            ...
            {CHECKLIST.map(({ label, key, check }) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${done ? "bg-emerald-500/20 border border-emerald-500/30 text-emerald-400" : "bg-slate-800 border border-white/[0.07] text-slate-600"}`}>
