            ...
            renderItem={(idea) => (
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-slate-200 text-xs font-semibold truncate">{idea.title}</p>
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-500/15 text-violet-400 capitalize flex-shrink-0">{idea.stage}</span>
                </div>
                <p className="text-slate-500 text-[11px] mt-0.5">{idea.authorName} · {idea.upvotes?.length ?? 0} upvotes</p>
              </div>
            )}
          />
        </div>
